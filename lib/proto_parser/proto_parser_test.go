...
func TestParseMessageWithRepeatedField(t *testing.T) {
	msg := `syntax = "proto3";

message PlaybackHeader {
	repeated uint32 magic = 1;
	float testfloat = 2;
	repeated double testdouble = 3;
}`
...
func TestParseMessageNoLineBreaks(t *testing.T) {
	msg := `syntax = "proto3"; message PlaybackHeader { repeated uint32 magic = 1; float testfloat = 2; repeated double testdouble = 3; }`
